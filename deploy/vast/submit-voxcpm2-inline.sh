...
WORKER_URL="${VAST_WORKER_URL:-${CUSTOM_WORKER_URL:-}}"
if [[ -z "${WORKER_URL}" ]]; then
  echo "Set VAST_WORKER_URL or CUSTOM_WORKER_URL in .env (from wait-instance.sh)" >&2
  exit 1
fi
WORKER_URL="${WORKER_URL%/}"

TEXT="${1:-សួស្តី! នេះជាការសាកល្បងសំឡេងខ្មែរលើ Vast.ai។}"
JOB_ID_LOCAL="khmer-vast-$(date +%s)"
REQUEST_FILE="$(mktemp)"
